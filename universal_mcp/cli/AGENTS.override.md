...
- **Reglas Fundamentales:**
  ...
  - Implementar el comando `umcp run <comando>` usando `subprocess.Popen` y `os.environ` para inyectar configuraciones dinámicas en el subproceso (el cliente IA de terceros) sin filtrar variables al sistema global.
  - Utilizar `Rich` para mostrar tablas de estado visualmente atractivas (`umcp status`).
